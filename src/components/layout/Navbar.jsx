...
const Navbar = () => {
  ...
  // Hydrate userLocation from localStorage after mount (SSR-safe)
  useEffect(() => {
      ...
      <nav
        className={`fixed w-full top-0 z-50 transition-all duration-500 ${
          isCustomer
            ...
            : isHomePage
            ? "bg-transparent"
            : "bg-white shadow-md shadow-orange-100/40"
        }`}
      ...
      >
            ...
            <div className="flex justify-between items-center w-full">
              ...
              <Link href="/" className="flex items-center gap-3 group" onClick={() => setMenuOpen(false)}>
                ...
                <div className="flex flex-col leading-tight">
                  <span className={`text-xl font-black tracking-tight ${isScrolled || !isHomePage ? "text-saffron-700" : "text-white"}`}>
                    The<span className="text-saffron-500">Tiffins</span>
                  </span>
                  <span className={`text-[10px] font-medium tracking-widest uppercase ${isScrolled || !isHomePage ? "text-gray-500" : "text-orange-200"}`}>
                    Home · Made · Fresh
                  </span>
                </div>
              </Link>
              ...
              <button
                className={`lg:hidden text-2xl p-2 rounded-lg transition-all duration-300 ${
                  isScrolled || !isHomePage ? "text-saffron-700 hover:bg-orange-50" : "text-white hover:bg-white/10"
                }`}
                  ...
                  <div className="relative">
                    ...
                    {dropdownOpen && (
                      <div className="absolute right-0 mt-2 w-52 bg-white rounded-2xl shadow-2xl border border-orange-100 overflow-hidden z-50">
                        ...
                        <div className="p-1">
                          <Link
                            href={userRole === 'customer' ? "/dashboard/customer" : "/dashboard/provider"}
                            onClick={() => setDropdownOpen(false)}
                            className="flex items-center gap-3 w-full px-4 py-3 text-sm text-gray-700 hover:bg-orange-50 hover:text-saffron-600 rounded-xl transition-all font-bold"
                          >
                           🏠 My Dashboard
        ...
        {!isCustomer && (
          <div
            ...
            } ${isScrolled || !isHomePage ? "bg-white" : "bg-gray-900/95 backdrop-blur-xl"}`}
          >
                          ...
                          {item.submenu.map((sub, si) => (
                            <Link
                              ...
                              className={`block px-4 py-2.5 rounded-xl text-sm ${
                                isScrolled || !isHomePage ? "text-gray-600 hover:text-saffron-600 hover:bg-orange-50" : "text-white/70 hover:text-white hover:bg-white/10"
                              }`}
                            >
                              {sub.name}
                            </Link>
